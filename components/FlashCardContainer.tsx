...
export default function FlashCardContainer() {
    ...
    const handleShowFlashCard = async (data: any): Promise<string> => {
      try {
        ...
        if (payload.action === "show") {
          ...
          setFlashCards((prev) =>
            prev.map((card) =>
              card.id === payload.id ? { ...card, isFlipped: !card.isFlipped } : card
            )
          );
  ...
  const handleFlip = async (id: string) => {
    try {
      ...
      if (agent) {
        ...
        setFlashCards((prev) =>
          prev.map((card) => (card.id === id ? { ...card, isFlipped: !card.isFlipped } : card))
        );
      } else {
  ...
  const currentCard =
    currentCardIndex !== null && flashCards[currentCardIndex] ? flashCards[currentCardIndex] : null;
        ...
        >
          ...
          {flashCards.length > 1 && (
            <div className="flex justify-between mt-4">
              <button
                onClick={() =>
                  setCurrentCardIndex((prev) => (prev !== null ? Math.max(0, prev - 1) : 0))
                }
